...
(64) 98124-9199`;
  ...
  const createAppointment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Atualizar o status localmente (simulação)
      setAppointments(prev => prev.map(apt => 
      ...
      setFormData({
        ...
        client_cpf: '',
        private_patient_id: '',
      setSuccess(`Status atualizado para: ${getStatusInfo(newStatus).text}`);
      });
      // Se confirmou a consulta, mostrar opção de WhatsApp
...
(64) 98124-9199`;
  ...
  const sendWhatsAppMessage = () => {
    ...
    // Usar um número padrão se não tiver telefone do cliente
    // Em um sistema real, você buscaria o telefone do cliente
    const defaultPhone = '5564981249199'; // Número do convênio
                        ...
                        {getAvailableActions(appointment.status).map((action) => (
                          ...
                          >
                            {action.icon}
                              ...
                              {/* Botão WhatsApp para consultas confirmadas */}
                              <button
