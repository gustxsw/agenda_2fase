...
const PaymentSection: React.FC<PaymentSectionProps> = ({ 
  ...
  useEffect(() => {
    const verifySubscriptionStatus = async () => {
      try {
        ...
        console.log('🔍 VERIFICAÇÃO DUPLA: Verificando status de assinatura para userId:', userId);
        console.log('🔍 Status recebido via props:', subscriptionStatus);

        const response = await fetch(`${apiUrl}/api/users/${userId}/subscription-status`, {
          method: 'GET',
          headers: {
        ...
        if (response.ok) {
          ...
          console.log('✅ Status verificado no servidor:', data.subscription_status);
        ...
        console.error('❌ Erro na verificação, usando status das props:', error);
        setVerifiedStatus(subscriptionStatus);
      } finally {
  ...
  const handlePayment = async () => {
    ...
    if (verifiedStatus === 'active') {
      ...
      setError('Sua assinatura já está ativa. Não é necessário realizar pagamento.');
      return;
    }
  ...
  console.log('⚠️ CLIENTE INATIVO: Mostrando opções de pagamento para status:', verifiedStatus);
