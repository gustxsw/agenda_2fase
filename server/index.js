  ...
  try {
    ...
    if (subscription_status === 'active') {
      ...
      return res.status(400).json({ 
        message: 'Sua assinatura já está ativa. Não é necessário realizar pagamento.',
        subscription_status,
        blocked: true
      });
    }
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      console.log('❌ User not found for CPF:', cleanCpf.substring(0, 3) + '***');
...
app.put('/api/users/:id', authenticate, async (req, res) => {
  try {
    ...
    if (professional_percentage !== undefined && req.user.currentRole === 'admin') {
...
app.put('/api/dependents/:id', authenticate, async (req, res) => {
  try {
    ...
    // Verify access
    if (req.user.id !== dependent.rows[0].client_id && req.user.currentRole !== 'admin') {
      return res.status(403).json({ message: 'Acesso negado' });
    }
...
app.post('/api/services', authenticate, authorize(['admin']), async (req, res) => {
  try {
    ...
    if (isNaN(base_price) || base_price <= 0) {
      return res.status(400).json({ message: 'Preço base deve ser um número maior que zero' });
    }
...
app.get('/api/consultations', authenticate, async (req, res) => {
  try {
    console.log('🔄 Fetching consultations for user:', req.user.id, 'role:', req.user.currentRole);
...
app.post('/api/consultations', authenticate, async (req, res) => {
  try {
    ...
    if (!client_id && !dependent_id && !private_patient_id) {
      return res.status(400).json({ message: 'Cliente, dependente ou paciente particular deve ser especificado' });
    }
...
app.put('/api/consultations/:id/status', authenticate, async (req, res) => {
  try {
    ...
    if (req.user.currentRole !== 'admin' && consultation.rows[0].professional_id !== req.user.id) {
      return res.status(403).json({ message: 'Acesso negado' });
    }
...
app.put('/api/attendance-locations/:id', authenticate, authorize(['professional', 'admin']), async (req, res) => {
  try {
    ...
    // If setting as default, remove default from other locations
    if (is_default) {
      const professionalId = req.user.currentRole === 'admin' 
        ? (await pool.query('SELECT professional_id FROM attendance_locations WHERE id = $1', [locationId])).rows[0]?.professional_id
        : req.user.id;
...
app.post('/api/medical-records/generate-document', authenticate, authorize(['professional', 'admin']), async (req, res) => {
  try {
...
app.get('/api/medical-documents', authenticate, authorize(['professional', 'admin']), async (req, res) => {
  try {
    console.log('🔄 Fetching medical documents for professional:', req.user.id);
...
app.get('/api/reports/professional-revenue', authenticate, authorize(['professional']), async (req, res) => {
  try {
    ...
    const professionalPercentage = parseInt(professionalData.rows[0]?.professional_percentage) || 50;

    console.log('🔍 Professional percentage from DB:', professionalPercentage);
...
app.post('/api/admin/grant-scheduling-access', authenticate, authorize(['admin']), async (req, res) => {
  try {
    ...
    console.log('🔄 Granting scheduling access:', { professional_id, expires_at });
...
app.post('/api/create-subscription', authenticate, preventPaymentForActiveClients, async (req, res) => {
  try {
    ...
    const preferenceData = {
      ...
      back_urls: {
        success: `${req.protocol}://${req.get('host')}/api/payment/success?type=subscription&user_id=${user_id}`,
        failure: `${req.protocol}://${req.get('host')}/api/payment/failure?type=subscription&user_id=${user_id}`,
        pending: `${req.protocol}://${req.get('host')}/api/payment/pending?type=subscription&user_id=${user_id}`
      },
...
app.post('/api/professional/create-payment', authenticate, authorize(['professional']), async (req, res) => {
  try {
    ...
    console.log('🔄 Creating professional payment for user:', req.user.id, 'amount:', amount);
...
app.get('/api/payment/success', async (req, res) => {
  try {
    ...
    console.log('✅ Payment success callback:', { type, user_id, dependent_id, professional_id });
...
app.get('/api/payment/failure', async (req, res) => {
  try {
    ...
    console.log('❌ Payment failure callback:', { type, user_id, dependent_id, professional_id });
...
app.get('/api/payment/pending', async (req, res) => {
  try {
    ...
    console.log('⏳ Payment pending callback:', { type, user_id, dependent_id, professional_id });
...
app.get('/api/system/stats', authenticate, authorize(['admin']), async (req, res) => {
  try {
    ...
    const systemStats = {
      ...
      medical: {
        total_private_patients: parseInt(stats.rows[0].total_private_patients) || 0,
        total_medical_records: parseInt(stats.rows[0].total_medical_records) || 0,
        total_medical_documents: parseInt(stats.rows[0].total_medical_documents) || 0
      }
    };
...
// 🔥 BACKUP ROUTE
app.post('/api/system/backup', authenticate, authorize(['admin']), async (req, res) => {
  try {
    ...
    for (const table of tables) {
      try {
        ...
        console.log(`✅ Backed up table ${table}: ${result.rows.length} records`);
      } catch (error) {
...
// 🔥 CLEANUP ROUTE
app.post('/api/system/cleanup', authenticate, authorize(['admin']), async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(`✅ Cleaned ${cleanupResults.old_audit_logs} old audit logs`);
    } catch (error) {
    ...
    try {
      ...
      console.log(`✅ Cleaned ${cleanupResults.orphaned_records} orphaned dependents`);
    } catch (error) {
...
app.post('/api/notifications', authenticate, authorize(['admin']), async (req, res) => {
  try {
    ...
    console.log('🔄 Creating notification:', { title, type, user_id, role_filter });
