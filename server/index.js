...
const initializeDatabase = async () => {
  try {
    ...
    if (adminCheck.rows.length === 0) {
      ...
      console.log(
        "✅ Admin user created - CPF: 00000000000, Password: admin123"
      );
    }
...
async function createTestProfessional() {
  try {
    ...
    const services = [
      { name: "Consulta Fisioterapêutica", price: 100.0 },
      { name: "Sessão de RPG", price: 80.0 },
      { name: "Massoterapia", price: 60.0 },
      { name: "Pilates Terapêutico", price: 90.0 },
    ];
    ...
    for (const service of services) {
      try {
        ...
        console.log(
          `⚠️ Serviço "${service.name}" já existe ou erro:`,
          error.message
        );
      }
    }
      ...
      try {
        ...
        if (create_appointment && appointment_date && appointment_time) {
          ...
          const consultationDuration =
            settingsResult.rows[0]?.consultation_duration || 60;
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const convenioConsultations = consultations.filter(
        (c) => c.patient_type === "convenio" || c.patient_type === "dependente"
      ).length;
      const privateConsultations = consultations.filter(
        (c) => c.patient_type === "particular"
      ).length;
      const convenioRevenue = consultations
        .filter(
          (c) =>
            c.patient_type === "convenio" || c.patient_type === "dependente"
        )
...
app.post(
  ...
  async (req, res) => {
    try {
      res.json({
        message:
          "O acesso à agenda está liberado para todos os profissionais e não pode ser revogado",
      });
    } catch (error) {
  ...
  async (req, res) => {
    try {
      if (!mpClient || !Preference) {
        return res
          .status(503)
          .json({ message: "Serviço de pagamento não disponível" });
      }
    ...
    try {
      ...
      const preferenceData = {
        ...
        payer: {
          name: req.user.name,
          email:
            req.user.email || `professional${req.user.id}@quiroferreira.com.br`,
        },
        back_urls: {
          success: `${
            process.env.FRONTEND_URL || "http://localhost:5173"
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const documentUrl = `${
        process.env.API_URL || "http://localhost:3001"
      }/documents/${Date.now()}_${document_type}.pdf`;
...
app.get(
  "/api/medical-records/patient/:patientId/:patientType",
  authenticate,
  authorize(["professional"]),
  async (req, res) => {
    try {
